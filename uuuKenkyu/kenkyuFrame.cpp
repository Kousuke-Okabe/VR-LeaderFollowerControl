...
kenkyu::Vector7 kenkyu::fjikkenWithGen(const Vector6& q,const Eigen::Quaterniond& gen) {
	...
	//実験装置に合わせたアーム()
	constexpr double l1 = 0.28, l2 = 0.35, l3 = 0.0;
	Affine3d trans = AngleAxisd(q(0), y) * AngleAxisd(q(1), z) * Translation<double, 3>(0, -l1, 0) * AngleAxisd(q(2), -z) * Translation<double, 3>(0, -l2, 0) * AngleAxisd(q(3), -x) * AngleAxisd(q(4), y) * Translation<double, 3>(0, -l3, 0) * AngleAxisd(q(5), -x);
