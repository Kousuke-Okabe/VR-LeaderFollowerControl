...
namespace _uuu {
	class textureLoaderByFreeImage{
	public:
		using formatStruct = std::pair<GLuint, GLuint>;//format,informat
		static void LoadTextureFromFile(const std::string& path, uuu::textureOperator& ref, boost::optional<size_t> texUnit = boost::none,boost::optional<formatStruct> formats=boost::none);
	};
};
